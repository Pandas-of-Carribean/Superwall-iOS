...
final class PaywallMessageHandler: WebEventDelegate {
    ...
    func handle(_ message: PaywallMessage) {
        ...
        case let .onReady(paywalljsVersion):
        ...
        case let .openUrl(url):
            openUrl(url)
        case let .openUrlInSafari(url):
            openUrlInSafari(url)
        case let .openDeepLink(url):
            openDeepLink(url)
        case .restore:
            restorePurchases()
        case let .purchase(productId: id):
            purchaseProduct(withId: id)
        case let .custom(data: name):
    ...
    private nonisolated func passTemplatesToWebView(from paywall: Paywall) async {
    ...
    /// feel native.
    private nonisolated func didLoadWebView(
        for paywall: Paywall,
        at loadedAt: Date
    ) async {
