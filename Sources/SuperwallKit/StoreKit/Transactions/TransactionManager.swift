...
final class TransactionManager {
    ...
    func purchase(_ purchaseSource: PurchaseSource) async -> PurchaseResult {
        ...
        case let .purchaseFunc(storeProduct):
            product = storeProduct
        case .observeFunc:
    ...
    func handle(
        ...
        case let .failed(error):
            ...
            guard
                ...
                case .observing:
                    break
                case let .purchasing(purchaseSource):
    ...
    func tryToRestore(_ restoreSource: RestoreSource) async -> RestorationResult {
        ...
        func handleRestoreResult(
            ...
            if hasRestored && hasActiveEntitlements {
                ...
                if case let .failed(error) = restorationResult,
                   let error = error
                {
        ...
        case let .internal(paywallViewController):
    ...
    func trackFailure(error: Error) async {
        ...
        switch source {
        case let .internal(_, paywallViewController):
            Logger.debug(
                logLevel: .debug,
                scope: .transactions,
